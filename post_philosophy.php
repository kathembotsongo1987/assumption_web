...
<div class="row">
    <?php      
      $select_stmt = $connection->prepare("SELECT * FROM post_philosophy"); //sql select query
      $select_stmt->execute();
      ...
      <li data-bs-target="#blogCarousel" data-bs-slide-to="1"></li>
            ...
            <div class="carousel-inner ">
              <div class="carousel-item active">
                <div class="row">
                  <?php
                      $select_stmt = $connection->prepare("SELECT * FROM post_philosophers limit 3 "); //sql select query
                        $select_stmt->execute();
                        while($row=$select_stmt->fetch(PDO::FETCH_ASSOC))
                        {
                        ?>
                ...
                <div class="carousel-item">
                  <div class="row">
                    <?php
                        $select_stmt = $connection->prepare("SELECT * FROM post_philosophers limit 3,5 "); //sql select query
                          $select_stmt->execute();
                          while($row=$select_stmt->fetch(PDO::FETCH_ASSOC)){
                          ?>
                    <div class="col-4">
...
<div><h1 style="text-align: center; padding: 20px; color: blue;">Current News</h1>
</div>
<div class="row">
    <?php      
      $select_stmt = $connection->prepare("SELECT * FROM post_philosophy_infor"); //sql select query
      $select_stmt->execute();
        while($row=$select_stmt->fetch(PDO::FETCH_ASSOC))
        {
        ?>
        <div class="col-4">
           <img style="width: 100%; height: 60%" src="admin/cruds/other_cruds/upload/<?php echo $row['photo1']; ?>">
            <h4 style="color: blue;"><?php echo $row['description1']; ?></h4>              
        </div>
        <div class="col-4">
            <h4 style="margin-left: 10px;  margin-right: 10px; text-align: justify;  overflow-y: scroll; overflow-x: scroll; height: 480px;"><?php echo $row['article']; ?></h4>
        </div>
        <div class="col-4">
          <img style="width: 100%; height: 60%" src="admin/cruds/other_cruds/upload/<?php echo $row['photo2']; ?>">
            <h4 style="color: blue;"><?php echo $row['description2']; ?></h4>       
        </div>    
        <?php 
