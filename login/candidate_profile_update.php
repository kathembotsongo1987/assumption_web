...
if(isset($_POST['update'])){
   ...
   $update_profile = $connection->prepare("UPDATE `religious_tb` SET name = ?, email = ? WHERE id = ?");
   $update_profile->execute([$name, $email, $candidate_id]);

   $old_image = $_POST['old_image'];
   $image = $_FILES['image']['name'];
   $image_tmp_name = $_FILES['image']['tmp_name'];
   $image_size = $_FILES['image']['size'];
   $image_folder = '../admin/cruds/other_cruds/upload/'.$image;
   ...
   if(!empty($image)){
      ...
      if($image_size > 2000000){
         ...
         $update_image = $connection->prepare("UPDATE `religious_tb` SET image = ? WHERE id = ?");
         $update_image->execute([$image, $candidate_id]);

         if($update_image){
            move_uploaded_file($image_tmp_name, $image_folder);
            unlink('../admin/cruds/other_cruds/upload/'.$old_image);
   ...
   if(!empty($previous_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($previous_pass != $old_pass){
         ...
         $update_password = $connection->prepare("UPDATE `religious_tb` SET password = ? WHERE id = ?");
         $update_password->execute([$confirm_pass, $candidate_id]);
...
<h1 class="title"> update <span><?= $fetch_profile['name']; ?></span> profile </h1>

<section class="update-profile-container">
   <form action="" method="post" enctype="multipart/form-data">
      <img src="../admin/cruds/other_cruds/upload/<?= $fetch_profile['image']; ?>" alt="">
      <div class="flex">
         <div class="inputBox">
            <span>username : </span>
      ...
      <div class="flex-btn">
         ...
         <a href="../admin/cruds/other_cruds/candidate.php" class="option-btn">go back</a>
      </div>
   </form>
