...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		$id = $_REQUEST['update_id']; //get "update_id from the read_lay.php page through anchor tag operation and store in "$id" variable
...
if(isset($_REQUEST['btn_update'])){
	try
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE lay SET image=:file_up, name=:name_up, country=:country_up, phone=:phone_up, email=:email_up, description=:description_up   WHERE id=:id'); //slq update the query.													
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:3; read_lay.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
