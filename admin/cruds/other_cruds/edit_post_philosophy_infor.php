...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM post_philosophy_infor WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE post_philosophy_infor SET photo1=:photo1, photo2=:photo2, description1=:description1, description2=:description2, article=:article WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("location: read_post_philosophy_infor.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
