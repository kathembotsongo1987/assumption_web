...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM noviciate WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE noviciate SET photo1=:photo1, photo2=:photo2, description1=:description1, description2=:description2, descriptionvideo=:descriptionvideo, video=:video WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("location: read_noviciate.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
								...
								<div class="col-sm-6">
									<label class="col-sm-6 control-label">Action</label>
										<input type="submit" name="btn_update" class="btn btn-success" value="update">
											<a href="read_noviciate.php" class="btn btn-danger">Cancel</a>
								</div>
							</div>
						</form>	
					</div> 
