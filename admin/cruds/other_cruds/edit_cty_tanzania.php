...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE communitiestanzania SET name=:name_up, diocese=:diocese_up WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("location: read_cty_tanzania.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
										...
										<div class="col-sm-6">
											<label class="col-sm-6 control-label">Action</label>
												<input type="submit" name="btn_update" class="btn btn-success" value="Update">
													<a href="read_cty_tanzania.php" class="btn btn-danger">Cancel</a>
										</div>
									</div>
							</form>
		 			</div>
