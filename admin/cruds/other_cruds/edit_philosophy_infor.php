...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM philosophy_infor WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE philosophy_infor SET photo1=:photo1, photo2=:photo2, description1=:description1, description2=:description2, article=:article WHERE id=:id'); //slq update the query.
			...
			$upldate_stmt->bindParam(':article',$article); // bind all parameter
			$upldate_stmt->bindParam(':id',$id);

			if($upldate_stmt->execute()){
				$updateMsg = "File Uplade Successfully....."; // File update success message
				header("location: read_philosophy_infor.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
			...
			<div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h1 style="text-align: center;">DATA MANIPULATION || PHILOSOPHY NEWS</h1>
                    </div>                       
						 ...
						 <form method="post" class="form-horizontal" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label">Article</label>
									<div class="col-sm-6">
										<input type="video" name="article" class="form-control" value=" <?php echo $article; ?> ">
									</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<label class="col-sm-6 control-label">Action</label>
										<input type="submit" name="btn_update" class="btn btn-success" value="update">
											<a href="read_philosophy_infor.php" class="btn btn-danger">Cancel</a>
								</div>
							</div>
						</form>	
					</div> 
