...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM slidesecumenism WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE slidesecumenism SET title=:title_up, smalltext=:description_up, image=:file_up WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("location: read_slides_ecumenism.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
		...
		<div class="col-lg-12">
			 <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <h3><a href="create_slides_ecumenism.php"><span class="glyphicon glyphicon-plus"></span>&nbsp; Add File</a></h3>
                    </div>
									...
									<div class="form-group">
										<div class="col-sm-offset-3 col-sm-9 m-t*15">
											<input type="submit" name="btn_update" class="btn btn-primary" value="Update">
												<a href="read_slidesecumenism.php" class="btn btn-danger">Cancel</a>
										</div>
									</div>
								</form>	
		 				</div>
