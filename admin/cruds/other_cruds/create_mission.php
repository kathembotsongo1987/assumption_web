	...
	<?php 	
		if(isset($_REQUEST['btn_insert']))
		{
			try
			{
				...
				if(empty($name)){
				...
				else if (empty($mission)) {
					$errorMsg = "please Enter the mission";	
				}
					...
					$errorMsg = "please Enter date for diaconate";	
						...
						$errorMsg = "Please Select Image";
				  		...
				  		if(!isset($errorMsg)){
				  			$insert_stmt = $connection->prepare('INSERT INTO mission(image, name, country, mission, community, temporary_vows, perpetual_vows, phone, email, diaconate_date, priesthood_date) VALUES(:image, :name, :country, :mission, :community, :temp_v, :perp_v, :phone, :email, :diaconate_date, :priesthood_date)'); // sql insert query
				  				
				  				$insert_stmt->bindParam(':image', $image_file); // bind all parameter
				  				$insert_stmt->bindParam(':name', $name);
				  				$insert_stmt->bindParam(':country', $country);
				  				$insert_stmt->bindParam(':mission', $mission);
				  				...
				  				if($insert_stmt->execute()){
				  					...
				  					header("location:read_mission.php"); //refresh 3 second and redirext to the read page
				  				}
				  		}
		...
		<div class="col-lg-12">
			<div class="panel panel-default">
                <div class="panel-heading">
                	<h1>DATA MANIPULATION || RELIGIOUS ON MISSION</h1>
                </div>
						...
						<form method="post" class="form-horizontal" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label">Mission</label>
									<div class="col-sm-6">
										<input type="text" name="mission" class="form-control" placeholder="enter name">
									</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<label class="col-sm-6 control-label">Action</label>
										<input type="submit" name="btn_insert" class="btn btn-success" value="Insert">
											<a href="read_mission.php" class="btn btn-danger">Cancel</a>
								</div>
							</div>
						</form>	
					</div>
