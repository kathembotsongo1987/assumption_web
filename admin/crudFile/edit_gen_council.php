...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		$message = $_REQUEST['txt_messsage']; // textbox name "txt_name"
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE general_council SET name=:name_up, message=:message_up, image=:file_up WHERE id=:id'); //slq update the query.
			$upldate_stmt->bindParam(':name_up',$name);
			$upldate_stmt->bindParam(':message_up',$message);
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:3; read_gen_council.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
...
<form method="post" class="form-horizontal" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<div class="col-sm-6">
			<input type="text" name="txt_messsage" class="form-control" value=" <?php echo $message; ?>" required/>
		</div>
	</div>
