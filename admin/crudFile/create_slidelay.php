			...
			<div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                           <h1 style="color: blue; text-align: center;">These are dynamic images for Lay assumptionists</h1>
                        </div>
                        <!-- /.panel-heading -->
				...
				if ($type=="image/jpg" || $type=='image/jpeg' || $type=='image/png' || $type=='image/gif') // check file extension
				  		{
				  			if(!file_exists($path)) // check file not exist in your upload folder part
				  			{
				  				...
				  				{
				  					move_uploaded_file($temp, "lay/upload/" .$image_file); // move upload file temperory directory to your upload foler
				  				} else {
				  		...
				  		if(!isset($errorMsg)){
				  			$insert_stmt = $connection->prepare('INSERT INTO slidelay(image) VALUES(:image)'); // sql insert query
				  				...
				  				if($insert_stmt->execute()){
				  					...
				  					header("refresh:3;read_slidelay.php"); //refresh 3 second and redirext to the read page
				  				}
				  		}
