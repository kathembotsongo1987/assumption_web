...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM upload_file WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE upload_file SET name=:name_up, image=:file_up WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:3; read.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
