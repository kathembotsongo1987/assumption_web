...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM triplelove WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE triplelove SET title=:title_up, description=:description_up, image=:file_up WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:3; read_triplelove.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
