	...
	try
			{
				...
				$intro_title = $_REQUEST['intro_title']; // this txt_name is the name of the input coming from the form	
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE lay_groups SET lay_image=:lay_image, intro_title=:intro_title, intro_description=:intro_description, articletitle=:articletitle, articledescription=:articledescription, video=:video WHERE id=:id'); //slq update the query.													
			$upldate_stmt->bindParam(':lay_image',$lay_image); // bind all parameter
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:1; read_lay_groups.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
...
<form method="post" class="form-horizontal" enctype="multipart/form-data">
	<div class="form-group">
		...
		<div class="col-sm-6">
			<input type="file" name="lay_image" class="form-control" value=" <?php echo $ecumenism_image; ?> " placeholder="enter Image">
		</div>
	</div>
