...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE novices SET image=:image, name=:name, country=:country, diocese=:diocese, parish=:parish WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:1; read_novices.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
								...
								<div class="col-sm-6">
									<label class="col-sm-6 control-label">Action</label>
										<input type="submit" name="btn_update" class="btn btn-success" value="update">
											<a href="read_novices.php" class="btn btn-danger">Cancel</a>
								</div>
							</div>
						</form>	
					</div> 
