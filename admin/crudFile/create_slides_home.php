							...
							<form method="post" class="form-horizontal" enctype="multipart/form-data">
								...
								<div class="form-group">
									<label class="col-sm-3 control-label">Big Text</label>
										<div class="col-sm-6">
											<input type="text" name="bigtext" class="form-control" placeholder="enter name">
										</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label">Small Text</label>
										<div class="col-sm-6">
											<input type="text" name="smalltext" class="form-control" placeholder="enter name">
										</div>
								</div>	
								<div class="form-group">
									<div class="col-sm-6">
			...
			{				
				...
				if(empty($image_file)){
						$errorMsg = "Please Select Image";
				  		...
				  		if(!isset($errorMsg)){
				  			$insert_stmt = $connection->prepare('INSERT INTO slides_home(Image, bigtext,smalltext) VALUES(:image, :bigtext, :smalltext)'); // sql insert query
				  				
				  				$insert_stmt->bindParam(':image', $image_file);
				  				$insert_stmt->bindParam(':bigtext', $bigtext);
				  				$insert_stmt->bindParam(':smalltext', $smalltext);
				  				
				  				if($insert_stmt->execute()){
				  					$insertMsg="File Upload Successfully."; // execute query success message
				  					header("refresh:1;read_slides_home.php"); //refresh 3 second and redirext to the read page
				  				}
				  		}
