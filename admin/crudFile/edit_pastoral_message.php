...
<?php 
 ...
 if(isset($_REQUEST['update_id'])) {
	try{
		...
		$select_stmt = $connection->prepare('SELECT * FROM pastoral_message WHERE id =:id'); //sql select query
...
if(isset($_REQUEST['btn_update'])){
	try {
		...
		$way="uploadformation/".$photo2; //set upload folder path
...
if($photo2){
			if($typ=="image/jpg" || $typ=='image/jpeg' || $typ=='image/png' || $typ=='image/gif') // check file extension
			{
				if(!file_exists($way)) // check file not exist in your upload folder path
				{
					if($siz < 5000000) // check file size 5MB
					{
						
						move_uploaded_file($tem, "uploadformation/".$photo2); //move upload file temporary directlry to your upload folder
					}
					else{
						$errorMsg = "Your File to Large please Upload 5MB size";
					}
			...
			$photo2 = $row['photo2']; //if not select new image than previous image sam it is it.
		}

		if(!isset($errorMsg)){
			$upldate_stmt = $connection->prepare('UPDATE pastoral_message SET photo1=:photo1, photo2=:photo2, description1=:description1, description2=:description2, descriptionvideo=:descriptionvideo, video=:video WHERE id=:id'); //slq update the query.
			...
			if($upldate_stmt->execute()){
				...
				header("refresh:1; read_pastoral_message.php");//reflesh 3 second and redirect to read.php page.
			}
		}
	}
	catch(PDOException $e){
			...
			<div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h1 style="color: blue; text-align: center;"> Create Pastoral Message</h1>
                    </div>                       
								...
								<div class="col-sm-6">
									<label class="col-sm-6 control-label">Action</label>
										<input type="submit" name="btn_update" class="btn btn-success" value="update">
											<a href="read_pastoral_message.php" class="btn btn-danger">Cancel</a>
								</div>
							</div>
						</form>	
					</div> 
